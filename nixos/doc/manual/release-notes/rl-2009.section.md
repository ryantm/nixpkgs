...
- Major module changes:
  ...
  - Two new options, [authorizedKeysCommand](#opt-services.openssh.authorizedKeysCommand) and [authorizedKeysCommandUser](#opt-services.openssh.authorizedKeysCommandUser), have been added to the `openssh` module. If you have `AuthorizedKeysCommand` in your [services.openssh.extraConfig](#opt-services.openssh.extraConfig) you should make use of these new options instead.
...
- Hardware:
  ...
  - [???](#opt-hardware.system76.firmware-daemon.enable) adds easy support of system76 firmware

  - [???](#opt-hardware.uinput.enable) loads uinput kernel module

  - [???](#opt-hardware.video.hidpi.enable) enable good defaults for HiDPI displays

  - [???](#opt-hardware.wooting.enable) support for Wooting keyboards

  - [???](#opt-hardware.xpadneo.enable) xpadneo driver for Xbox One wireless controllers

- Programs:

  - [???](#opt-programs.hamster.enable) enable hamster time tracking

  - [???](#opt-programs.steam.enable) adds easy enablement of steam and related system configuration

- Security:

  - [???](#opt-security.doas.enable) alternative to sudo, allows non-root users to execute commands as root

  - [???](#opt-security.tpm2.enable) add Trusted Platform Module 2 support

- System:

  - [???](#opt-boot.initrd.network.openvpn.enable) start an OpenVPN client during initrd boot

- Virtualization:

  - [???](#opt-boot.enableContainers) use nixos-containers

  - [???](#opt-virtualisation.oci-containers.containers) run OCI (Docker) containers

  - [???](#opt-virtualisation.podman.enable) daemonless container engine

- Services:

  - [???](#opt-services.ankisyncd.enable) Anki sync server

  - [???](#opt-services.bazarr.enable) Subtitle manager for Sonarr and Radarr

  - [???](#opt-services.biboumi.enable) Biboumi XMPP gateway to IRC

  - [???](#opt-services.blockbook-frontend) Blockbook-frontend, a service for the Trezor wallet

  - [???](#opt-services.cage.enable) Wayland cage service

  - [???](#opt-services.convos.enable) IRC daemon, which can be accessed throught the browser

  - [???](#opt-services.engelsystem.enable) Tool for coordinating volunteers and shifts on large events

  - [???](#opt-services.espanso.enable) text-expander written in rust

  - [???](#opt-services.foldingathome.enable) Folding\@home client

  - [???](#opt-services.gerrit.enable) Web-based team code collaboration tool

  - [???](#opt-services.go-neb.enable) Matrix bot

  - [???](#opt-services.hardware.xow.enable) xow as a systemd service

  - [???](#opt-services.hercules-ci-agent.enable) Hercules CI build agent

  - [???](#opt-services.jicofo.enable) Jitsi Conference Focus, component of Jitsi Meet

  - [???](#opt-services.jirafeau.enable) A web file repository

  - [???](#opt-services.jitsi-meet.enable) Secure, simple and scalable video conferences

  - [???](#opt-services.jitsi-videobridge.enable) Jitsi Videobridge, a WebRTC compatible router

  - [???](#opt-services.jupyterhub.enable) Jupyterhub development server

  - [???](#opt-services.k3s.enable) Lightweight Kubernetes distribution

  - [???](#opt-services.magic-wormhole-mailbox-server.enable) Magic Wormhole Mailbox Server

  - [???](#opt-services.malcontent.enable) Parental Control support

  - [???](#opt-services.matrix-appservice-discord.enable) Matrix and Discord bridge

  - [???](#opt-services.mautrix-telegram.enable) Matrix-Telegram puppeting/relaybot bridge

  - [???](#opt-services.mirakurun.enable) Japanese DTV Tuner Server Service

  - [???](#opt-services.molly-brown.enable) Molly-Brown Gemini server

  - [???](#opt-services.mullvad-vpn.enable) Mullvad VPN daemon

  - [???](#opt-services.ncdns.enable) Namecoin to DNS bridge

  - [???](#opt-services.nextdns.enable) NextDNS to DoH Proxy service

  - [???](#opt-services.nix-store-gcs-proxy) Google storage bucket to be used as a nix store

  - [???](#opt-services.onedrive.enable) OneDrive sync service

  - [???](#opt-services.pinnwand.enable) Pastebin-like service

  - [???](#opt-services.pixiecore.enable) Manage network booting of machines

  - [???](#opt-services.privacyidea.enable) Privacy authentication server

  - [???](#opt-services.quorum.enable) Quorum blockchain daemon

  - [???](#opt-services.robustirc-bridge.enable) RobustIRC bridge

  - [???](#opt-services.rss-bridge.enable) Generate RSS and Atom feeds

  - [???](#opt-services.rtorrent.enable) rTorrent service

  - [???](#opt-services.smartdns.enable) SmartDNS DNS server

  - [???](#opt-services.sogo.enable) SOGo groupware

  - [???](#opt-services.teeworlds.enable) Teeworlds game server

  - [???](#opt-services.torque.mom.enable) torque computing node

  - [???](#opt-services.torque.server.enable) torque server

  - [???](#opt-services.tuptime.enable) A total uptime service

  - [???](#opt-services.urserver.enable) X11 remote server

  - [???](#opt-services.wasabibackend.enable) Wasabi backend service

  - [???](#opt-services.yubikey-agent.enable) Yubikey agent

  - [???](#opt-services.zigbee2mqtt.enable) Zigbee to MQTT bridge
...
```
  If you use MySQL instead of MariaDB please replace `unix_socket` with `auth_socket`. If you have changed the value of [???](#opt-services.mysql.user) from the default of `mysql` to a different user please change `'mysql'@'localhost'` to the corresponding user instead.
...
- The [matrix-synapse](#opt-services.matrix-synapse.enable) module no longer includes optional dependencies by default, they have to be added through the [plugins](#opt-services.matrix-synapse.plugins) option.
...
```
- The [supybot](#opt-services.supybot.enable) module now uses `/var/lib/supybot` as its default [stateDir](#opt-services.supybot.stateDir) path if `stateVersion` is 20.09 or higher. It also enables a number of [systemd sandboxing options](https://www.freedesktop.org/software/systemd/man/systemd.exec.html#Sandboxing) which may possibly interfere with some plugins. If this is the case you can disable the options through attributes in `systemd.services.supybot.serviceConfig`.

- The `security.duosec.skey` option, which stored a secret in the nix store, has been replaced by a new [security.duosec.secretKeyFile](#opt-security.duosec.secretKeyFile) option for better security.

  `security.duosec.ikey` has been renamed to [security.duosec.integrationKey](#opt-security.duosec.integrationKey).
...
- The NixOS options `nesting.clone` and `nesting.children` have been deleted, and replaced with named [???](#opt-specialisation) configurations.
...
- The httpd web server previously started its main process as root privileged, then ran worker processes as a less privileged identity user. This was changed to start all of httpd as a less privileged user (defined by [???](#opt-services.httpd.user) and [???](#opt-services.httpd.group)). As a consequence, all files that are needed for httpd to run (included configuration fragments, SSL certificates and keys, etc.) must now be readable by this less privileged user/group.

  The default value for [???](#opt-services.httpd.mpm) has been changed from `prefork` to `event`. Along with this change the default value for [services.httpd.virtualHosts.\<name\>.http2](#opt-services.httpd.virtualHosts) has been set to `true`.
...
- The `systemd-networkd` option `systemd.network.networks._name_.dhcpConfig` has been renamed to [???](#opt-systemd.network.networks._name_.dhcpV4Config) following upstream systemd\'s documentation change. See systemd.network 5 for details.

- In the `picom` module, several options that accepted floating point numbers encoded as strings (for example [???](#opt-services.picom.activeOpacity)) have been changed to the (relatively) new native `float` type. To migrate your configuration simply remove the quotes around the numbers.
...
- In the `resilio` module, [???](#opt-services.resilio.httpListenAddr) has been changed to listen to `[::1]` instead of `0.0.0.0`.

- `sslh` has been updated to version `1.21`. The `ssl` probe must be renamed to `tls` in [???](#opt-services.sslh.appendConfig).
...
- We now have a unified [???](#opt-services.xserver.displayManager.autoLogin) option interface to be used for every display-manager in NixOS.
...
- The [???](#opt-services.postgresql.dataDir) option is now set to `"/var/lib/postgresql/${cfg.package.psqlSchema}"` regardless of your [???](#opt-system.stateVersion). Users with an existing postgresql install that have a [???](#opt-system.stateVersion) of `17.03` or below should double check what the value of their [???](#opt-services.postgresql.dataDir) option is (`/var/db/postgresql`) and then explicitly set this value to maintain compatibility:
...
```
  The postgresql module now expects there to be a database super user account called `postgres` regardless of your [???](#opt-system.stateVersion). Users with an existing postgresql install that have a [???](#opt-system.stateVersion) of `17.03` or below should run the following SQL statements as a database super admin user before upgrading:
...
- The package nextcloud17 has been removed and nextcloud18 was marked as insecure since both of them will [ will be EOL (end of life) within the lifetime of 20.09](https://docs.nextcloud.com/server/19/admin_manual/release_schedule.html).
  ...
  - From nextcloud17, you have to upgrade to nextcloud18 first as Nextcloud doesn\'t allow going multiple major revisions forward in a single upgrade. This is possible by setting [???](#opt-services.nextcloud.package) to nextcloud18.

  - From nextcloud18, it\'s possible to directly upgrade to nextcloud19 by setting [???](#opt-services.nextcloud.package) to nextcloud19.
...
- The notmuch package move its emacs-related binaries and emacs lisp files to a separate output. They\'re not part of the default `out` output anymore - if you relied on the `notmuch-emacs-mua` binary or the emacs lisp files, access them via the `notmuch.emacs` output. Device tree overlay support was improved in [\#79370](https://github.com/NixOS/nixpkgs/pull/79370) and now uses [???](#opt-hardware.deviceTree.kernelPackage) instead of `hardware.deviceTree.base`. [???](#opt-hardware.deviceTree.overlays) configuration was extended to support `.dts` files with symbols. Device trees can now be filtered by setting [???](#opt-hardware.deviceTree.filter) option.
...
- With this release `systemd-networkd` (when enabled through [???](#opt-networking.useNetworkd)) has it\'s netlink socket created through a `systemd.socket` unit. This gives us control over socket buffer sizes and other parameters. For larger setups where networkd has to create a lot of (virtual) devices the default buffer size (currently 128MB) is not enough.
  ...
  Since the actual memory requirements depend on hardware, timing, exact configurations etc. it isn\'t currently possible to infer a good default from within the NixOS module system. Administrators are advised to monitor the logs of `systemd-networkd` for `rtnl: kernel receive buffer
...
- Specifying [mailboxes](#opt-services.dovecot2.mailboxes) in the dovecot2 module as a list is deprecated and will break eval in 21.05. Instead, an attribute-set should be specified where the `name` should be the key of the attribute.
...
- The [nix.buildMachines](#opt-nix.buildMachines) option is now type-checked. There are no functional changes, however this may require updating some configurations to use correct types for all attributes.
